...
size_t OptionsTab::CPlayerSettingsHelper::getImageIndex()
{
	...
	case TOWN:
		...
		case PlayerSettings::RANDOM:
			return TOWN_RANDOM;
		default:
			return (*CGI->townh)[settings.castle]->town->clientInfo.icons[true][false] + 2;
		}
	...
	case BONUS:
	{
		...
		{
			switch((*CGI->townh)[settings.castle]->town->primaryRes)
			{
...
std::string OptionsTab::CPlayerSettingsHelper::getName()
{
	...
	case TOWN:
	{
		...
		default:
			return (*CGI->townh)[settings.castle]->name;
		}
	}
	case HERO:
	{
...
void OptionsTab::CPlayerOptionTooltipBox::genTownWindow()
{
	...
	const CTown * town = (*CGI->townh)[settings.castle]->town;
